	...
	for _, file := range files {
		...
		for idx := 0; idx < maxNumEntries; idx++ {
			...
			if entry.Type() == uint64(raftpb.EntryNormal) {
				...
				mmap.ZeroOut(file.Data, offset, offset+4)
			}
		}
	}
}
...
func (l *wal) AddEntries(entries []raftpb.Entry) error {
	...
	if eidx >= 0 {
		if fidx == -1 {
			...
			if l.nextEntryIdx > eidx {
				mmap.ZeroOut(l.current.Data, entrySize*eidx, entrySize*l.nextEntryIdx)
			}
		} else {
			...
			if fidx >= len(l.files) {
				panic("fidx must larger than file number")
			}
			...
			for _, ef := range extra {
				l.logger.Info("deleting extra file", zap.Int64("fid", ef.fid), zap.String("filename", ef.Fd.Name()))

				if err := ef.delete(); err != nil {
					l.logger.Error("deleting file failed", zap.Error(err), zap.String("filename", ef.Fd.Name()))
				}
			}
			mmap.ZeroOut(l.current.Data, entrySize*eidx, logFileOffset)
	...
	for _, re := range entries {
		...
		if copy(destBuf, re.Data) != len(re.Data) {
			panic("entry data not copied")
		}
...
func (l *wal) rotate(firstIndex uint64, offset int) error {
	...
	ef, err := openLogFile(l.dir, nextFid, l.logger)
	if err != nil {
...
func openWal(dir string, logger *zap.Logger) (*wal, error) {
	w := &wal{
		dir:    dir,
		logger: logger.With(zap.String("wal", dir)),
	}
	files, err := getLogFiles(dir, w.logger)
	if err != nil {
		return nil, err
	}
	out := files[:0]
	...
	w.files = out
	if sz := len(w.files); sz > 0 {
		w.current = w.files[sz-1]
		w.nextEntryIdx = w.current.firstEmptySlot()

		w.files = w.files[:sz-1]
		return w, nil
	}
