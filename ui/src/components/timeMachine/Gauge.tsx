...
// Constants
import { GAUGE_THEME_DARK, GaugeTheme } from 'constants/gauge';
import {
  COLOR_TYPE_MIN,
  COLOR_TYPE_MAX,
  DEFAULT_VALUE_MAX,
...
import { DecimalPlaces } from '../../types/Dashboard';
import { MAX_DECIMAL_PLACES } from '../../constants/dashboard';
import { preventNegativeZero } from '../../utils/preventNegativeZero';
...
class Gauge extends Component<Props> {
  ...
  //@ts-ignore
  private drawSegmentedGauge = (ctx, xc, yc, r, minValue, maxValue, gradientThickness
  ) => {
    const { colors } = this.props;
    const sortedColors = _.sortBy(colors, color => Number(color.value));
  ...
  //@ts-ignore
  private drawGaugeLabels = (ctx, radius, gradientThickness, minValue, maxValue ) => {
