...
const useCache = ({ persist, cacheLife, cachePolicy }: UseCacheArgs): Cache => {
  ...
  invariant(
    !(isServer && persist),
    'There is no persistent storage on the Server currently! 🙅‍♂️'
  );
  invariant(
    !(isNative && persist),
    'React Native support for persistent cache is not yet implemented. 🙅‍♂️'
  );
  invariant(
    !(persist && [NO_CACHE, NETWORK_ONLY].includes(cachePolicy)),
    `You cannot use option 'persist' with cachePolicy: ${cachePolicy} 🙅‍♂️`
  );
