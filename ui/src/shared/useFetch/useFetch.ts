...
function useFetch<TData = any>(...args: UseFetchArgs): UseFetch<TData> {
  ...
  const makeFetch = useDeepCallback(
      ...
      const doFetch = async (
        ...
        try {
          ...
          res.current = interceptors.response
            ? await interceptors.response({response: res.current})
            : res.current
          invariant(
            'data' in res.current,
