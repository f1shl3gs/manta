...
export default function useFetchArgs(
  urlOrPathOrOptionsOrOverwriteGlobalOptions?:
    ...
    | IncomingOptions
    | OverwriteGlobalOptions,
  optionsOrOverwriteGlobalOrDeps?:
    | IncomingOptions
    | OverwriteGlobalOptions
    | any[],
  deps?: any[]
  ...
  invariant(
    !(
      isObject(urlOrPathOrOptionsOrOverwriteGlobalOptions) &&
      isObject(optionsOrOverwriteGlobalOrDeps)
    ),
  ...
  const overwriteGlobalOptions = useMemo(():
    | OverwriteGlobalOptions
    | undefined => {
    if (isFunction(urlOrPathOrOptionsOrOverwriteGlobalOptions))
      return urlOrPathOrOptionsOrOverwriteGlobalOptions as OverwriteGlobalOptions
    if (isFunction(optionsOrOverwriteGlobalOrDeps))
      return optionsOrOverwriteGlobalOrDeps as OverwriteGlobalOptions
  }, [])
  ...
  invariant(
    isFunction(retryDelay) ||
      (Number.isInteger(retryDelay as number) && retryDelay >= 0),
    '`retryDelay` must be a positive number or a function returning a positive number.'
  )
  const isValidRetryOn =
    isFunction(retryOn) ||
    (Array.isArray(retryOn) && retryOn.every(isPositiveNumber))
  invariant(
    isValidRetryOn,
  ...
  const customOptions = useMemo((): CustomOptions => {
    ...
    ) as (keyof CustomOptions)[] // Array<keyof CustomOptions>
