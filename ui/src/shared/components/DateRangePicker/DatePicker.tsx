...
const isValidRTC3339 = (d: string): boolean => {
  return (
    dayjs(d, 'YYYY-MM-DD HH:mm', true).isValid() ||
    dayjs(d, 'YYYY-MM-DD HH:mm:ss', true).isValid() ||
    dayjs(d, 'YYYY-MM-DD HH:mm:ss.SSS', true).isValid() ||
    dayjs(d, 'YYYY-MM-DD', true).isValid()
  )
}
...
const getFormat = (d: string): string => {
  ...
  if (dayjs(d, 'YYYY-MM-DD HH:mm', true).isValid()) {
    return 'YYYY-MM-DD HH:mm'
  }
  if (dayjs(d, 'YYYY-MM-DD HH:mm:ss', true).isValid()) {
    return 'YYYY-MM-DD HH:mm:ss'
  }
  if (dayjs(d, 'YYYY-MM-DD HH:mm:ss.SSS', true).isValid()) {
...
export default class DatePicker extends PureComponent<Props, State> {
  ...
  private get inputValue(): string {
    ...
    if (inputFormat) {
      return dayjs(dateTime).format(inputFormat)
    }
