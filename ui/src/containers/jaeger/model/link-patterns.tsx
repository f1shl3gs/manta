...
import {TNil, TraceSpan, TraceLink, TraceKeyValuePair, Trace} from '../types'
...
export function getParameterInAncestor(name: string, span: TraceSpan) {
  ...
  while (currentSpan) {
    const result =
      getParameterInArray(name, currentSpan.tags) ||
      getParameterInArray(name, currentSpan.process.tags)
    if (result) {
      return result
    }
...
export function createGetLinks(
  linkPatterns: ProcessedLinkPattern[],
  cache: WeakMap<TraceKeyValuePair, TraceLink[]>
) {
...
const processedLinks: ProcessedLinkPattern[] = (
  getConfigValue('linkPatterns') || []
)
