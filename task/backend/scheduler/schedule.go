...
func NewSchedule(unparsed string, lastScheduledAt time.Time) (Schedule, time.Time, error) {
	...
	if strings.HasPrefix(unparsed, "@every ") {
		...
		every, err := duration.Parse(everyString)
		if every < 0 {
			return Schedule{}, time.Time{}, errors.New("duration must be positive")
		}
		...
		// and align
		lastScheduledAt = lastScheduledAt.Truncate(every).Truncate(time.Second)
	}
