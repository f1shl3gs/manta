...
func TestRun(t *testing.T) {
	sch, _, err := NewScheduler(&mockExecutor{fn: func(l *sync.Mutex, ctx context.Context, id ID, scheduledFor time.Time) {
		fmt.Println("me", id)
	}}, &mockSchedulableService{fn: func(ctx context.Context, id ID, t time.Time) error {
		fmt.Println("mss", id)
		return nil
	...
	task, ts, err := NewSchedule("@every 3s", time.Now())
	require.NoError(t, err)
	fmt.Println("ts", ts)

	err = sch.Schedule(mockSchedulable{id: 1, schedule: task, offset: time.Second, lastScheduled: ts.UTC()})
	require.NoError(t, err)
